...
public class DangKyKhoaHocCreateRequestValidator : AbstractValidator<DangKyKhoaHocCreateRequest>
{
    private readonly IUnitOfWork _unitOfWork;

    public DangKyKhoaHocCreateRequestValidator(IUnitOfWork unitOfWork)
    {
    ...
    private async Task<bool> NotAlreadyRegistered(DangKyKhoaHocCreateRequest request, Guid hocVienId, CancellationToken cancellationToken)
    {
        var exists = await _unitOfWork.DangKyKhoaHocs.ExistsAsync(x => x.HocVienId == hocVienId && x.KhoaHocId == request.KhoaHocId);
        return !exists;
    }
}
