...
public class HocVienRequestValidator : AbstractValidator<HocVienRequest>
{
    private readonly IUnitOfWork _unitOfWork;

    public HocVienRequestValidator(IUnitOfWork unitOfWork)
    {
        ...
        RuleFor(x => x.TaiKhoanId)
            ...
            .MustAsync(IsUniqueTaiKhoanId).WithMessage("Tài khoản đã được liên kết với học viên khác");
    }
    ...
    {
        var existingHocVien = await _unitOfWork.HocViens.GetAllAsync(hv => hv.TaiKhoanId == taiKhoanId);
        return existingHocVien == null || !existingHocVien.Any();
    }
}
